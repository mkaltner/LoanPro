...
public class Calculator {
    ...
    public int addNumberToScreen(int number) {    	
    	if (newValue == false || clearScreen) {
    		screenValue = (double)number;
    		numberMode = Constants.MODE_INT;
    		currentPrecision = 0;
    	}
    	else {
    		if (numberMode == Constants.MODE_INT) {
    			screenValue = Double.parseDouble("" + screenValue.intValue() + number);
    		}
    		else {
    			String value = "";
    			if (currentPrecision == 0) {
    				value += screenValue.intValue();
    				value += ".";
    				value += number;
    				buffer = value;
    			}
    			...
    			screenValue = Double.parseDouble(buffer);
    			currentPrecision++;
    		}
    	}
    	...
    	if (newValue && !clearScreen) {
    		...
    		if (value.length() > 0) {
    			...
    			if (value.length() == 0) {
    				...
    				screenValue = Double.parseDouble(value);
    			}
    		}
    	}
    ...
    public int makePercent() {
    	...
    	if (performingCalculation) {
    		...
    		screenValue /= 100;
    		precision = screenValue.toString().split("\\.")[1].length();
    	}
